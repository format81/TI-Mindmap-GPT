...
if st.session_state.show_tabs:
    ...
    with tab1:  
            ...
            if "not related to cybersecurity" in relevance_check:
                ...
                if submit_cb_ioc:
                    with st.spinner("Extracting IOCs"):
                        ...
                        if st.session_state['iocs_df']:  
                            ...
                            st.session_state['iocs_df'] = iocs_df
                ...
                if submit_cb_ttps_timeline:
                    with st.spinner("Mermaid TTPs Timeline"):
                        ...
                        st.session_state['mermaid_timeline'] = mermaid_timeline
        ...
        # TAB3 - PDF Report
        with tab3: # Or st.tabs(["...", "..."])[index_for_tab3] if that's how you define it
            st.header("📈 PDF Report")
            
            # Form for PDF generation options
            with st.form("form_pdf_generation"):
                st.markdown("Configure your PDF report content below.")

                # PDF Orientation Selection
                pdf_orientation_choice = st.selectbox(
                    "PDF Orientation:",
                    options=["portrait", "landscape"],
                    index=0, # Default to portrait
                    help="Choose the page orientation for your PDF report."
                )
                
                # Checkbox to confirm inclusion of key elements (optional, as most are now passed)
                # You can add more specific checkboxes here if you want users to toggle sections.
                # For now, we assume if data exists, it will be included.
                # submit_cb_summary_pdf = st.checkbox("Include Summary and Main MindMap", value=True, key="pdf_sum_mm")
                # submit_cb_iocs_pdf = st.checkbox("Include IOCs Table", value=True, key="pdf_iocs")
                # submit_cb_ttps_all_pdf = st.checkbox("Include All TTPs Sections", value=True, key="pdf_ttps_all")
                # submit_cb_5whats_pdf = st.checkbox("Include 5 Whats Report", value=True, key="pdf_5whats")


                # Submit button for PDF generation
                submit_button_pdf = st.form_submit_button(
                    label=":orange[**Generate PDF Report**]",
                    help="Click to generate the PDF with the selected content and orientation."
                )

            if submit_button_pdf:
                if client: # Check if AI client is initialized (might be needed for some checks or future enhancements)
                    
                    # --- 1. Gather all necessary data from st.session_state ---
                    source_url = st.session_state.get('url4', "N/A")
                    summary_text = st.session_state.get('summary', "")
                    mindmap_code_main = st.session_state.get('mindmap_code', "")
                    
                    iocs_dataframe = st.session_state.get('iocs_df', None)
                    ttps_overview_text = st.session_state.get('ttptable', "") # This is the TTPs table/overview
                    attack_path_text = st.session_state.get('attackpath', "") # TTPs ordered by execution time
                    
                    # Ensure 'mermaid_timeline' is correctly populated in session_state from tab1
                    # when 'submit_cb_ttps_timeline' is checked.
                    # Example: st.session_state['mermaid_timeline'] = generated_mermaid_timeline_code
                    ttp_timeline_mermaid_code = st.session_state.get('mermaid_timeline', "")
                    
                    five_whats_text = st.session_state.get('5whats', "")

                    # --- 2. Basic check: Ensure there's at least some core content ---
                    if not (summary_text or mindmap_code_main or iocs_dataframe is not None or ttps_overview_text or attack_path_text or ttp_timeline_mermaid_code or five_whats_text) :
                        st.warning("No content available to generate a PDF. Please process data in the 'Main' tab first.")
                    else:
                        st.info(f"Generating PDF in {pdf_orientation_choice} orientation...")
                        with st.spinner("Crafting your PDF report, please wait..."):
                            # --- 3. Call the updated ti_pdf.create_pdf_bytes function ---
                            pdf_bytes = ti_pdf.create_pdf_bytes(
                                url=source_url,
                                summary_content=summary_text,
                                mindmap_mermaid_code=mindmap_code_main,
                                iocs_data=iocs_dataframe,
                                ttps_overview_data=ttps_overview_text, # Pass the TTPs table/overview text
                                attack_path_data=attack_path_text,    # Pass TTPs ordered by time
                                mermaid_timeline_code=ttp_timeline_mermaid_code,
                                five_whats_data=five_whats_text,
                                orientation=pdf_orientation_choice # Pass the user's choice
                            )

                            # --- 4. Provide download button or error message ---
                            if pdf_bytes:
                                ...
                                st.success(f"PDF report '{pdf_file_name}' generated successfully and is ready for download!")
                            else:
                                st.error("Failed to generate PDF report. An error occurred during PDF creation. Check application logs if available.")
                else:
                    st.error("AI Service client not initialized. Please configure it in the sidebar first.")
